...
func testProductToManyInvoiceProducts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadInvoiceProducts(ctx, tx, false, (*[]*Product)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testProductToManyAddOpInvoiceProducts(t *testing.T) {
	var err error
...
var (
	productDBTypes = map[string]string{`ID`: `uuid`, `Name`: `character varying`, `Description`: `character varying`, `Amount`: `integer`, `Slot`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
