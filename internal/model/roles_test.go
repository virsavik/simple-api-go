...
func testRolesExists(t *testing.T) {
	...
	e, err := RoleExists(ctx, tx, o.Name)
	if err != nil {
...
func testRolesFind(t *testing.T) {
	...
	roleFound, err := FindRole(ctx, tx, o.Name)
	if err != nil {
		t.Error(err)
	}
...
func testRoleToManyRoleNameUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.RoleNameUsers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.RoleName == b.RoleName {
			bFound = true
		}
		if v.RoleName == c.RoleName {
			cFound = true
		}
	}
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadRoleNameUsers(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RoleNameUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.RoleNameUsers = nil
	if err = a.L.LoadRoleNameUsers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RoleNameUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testRoleToManyAddOpRoleNameUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddRoleNameUsers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.Name != first.RoleName {
			t.Error("foreign key was wrong value", a.Name, first.RoleName)
		}
		if a.Name != second.RoleName {
			t.Error("foreign key was wrong value", a.Name, second.RoleName)
		}

		if first.R.RoleNameRole != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.RoleNameRole != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.RoleNameUsers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.RoleNameUsers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.RoleNameUsers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	roleDBTypes = map[string]string{`Name`: `character varying`}
	_           = bytes.MinRead
)
