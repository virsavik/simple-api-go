...
var RoleRels = struct {
	...
	RoleNameUsers: "RoleNameUsers",
}

// roleR is where relationships are stored.
type roleR struct {
	RoleNameUsers UserSlice `boil:"RoleNameUsers" json:"RoleNameUsers" toml:"RoleNameUsers" yaml:"RoleNameUsers"`
}
...
func (r *roleR) GetRoleNameUsers() UserSlice {
	if r == nil {
		return nil
	}
	return r.RoleNameUsers
}
...
func (q roleQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Role, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: failed to execute a one query for roles")
	}
...
// RoleNameUsers retrieves all the user's Users with an executor via role_name column.
func (o *Role) RoleNameUsers(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"users\".\"role_name\"=?", o.Name),
	)

	return Users(queryMods...)
}

// LoadRoleNameUsers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (roleL) LoadRoleNameUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(
		qm.From(`users`),
		qm.WhereIn(`users.role_name in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.RoleNameUsers = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userR{}
			}
			foreign.R.RoleNameRole = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.Name == foreign.RoleName {
				local.R.RoleNameUsers = append(local.R.RoleNameUsers, foreign)
				if foreign.R == nil {
					foreign.R = &userR{}
				}
				foreign.R.RoleNameRole = local
				break
			}
		}
	}
...
// AddRoleNameUsers adds the given related objects to the existing relationships
// of the role, optionally inserting them as new records.
// Appends related to o.R.RoleNameUsers.
// Sets related.R.RoleNameRole appropriately.
func (o *Role) AddRoleNameUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	var err error
	...
	if o.R == nil {
		o.R = &roleR{
			RoleNameUsers: related,
		}
	} else {
		o.R.RoleNameUsers = append(o.R.RoleNameUsers, related...)
	}
...
func FindRole(ctx context.Context, exec boil.ContextExecutor, name string, selectCols ...string) (*Role, error) {
	roleObj := &Role{}
	...
	query := fmt.Sprintf(
		"select %s from \"roles\" where \"name\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "model: unable to select from roles")
	}
...
func (o *Role) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to insert into roles")
	}
...
func (o *Role) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("model: unable to update roles, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by update for roles")
	}
...
func (q roleQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected for roles")
	}
...
func (o RoleSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: unable to retrieve rows affected all in update all role")
	}
	return rowsAff, nil
}
...
func (o *Role) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("model: unable to upsert roles, could not build update column list")
		}
...
func (o *Role) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"roles\" WHERE \"name\"=$1"
	...
	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by delete for roles")
	}
...
func (q roleQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "model: failed to get rows affected by deleteall for roles")
	}
...
func (o *Role) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindRole(ctx, exec, o.Name)
	if err != nil {
		return err
	}
...
func RoleExists(ctx context.Context, exec boil.ContextExecutor, name string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"roles\" where \"name\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "model: unable to check if roles exists")
	}
