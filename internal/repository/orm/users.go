...
var UserWhere = struct {
	...
	RoleName:  whereHelperstring{field: "\"users\".\"role_name\""},
...
var UserRels = struct {
	RoleNameRole        string
	StorekeeperInvoices string
}{
	RoleNameRole:        "RoleNameRole",
	StorekeeperInvoices: "StorekeeperInvoices",
}

// userR is where relationships are stored.
type userR struct {
	RoleNameRole        *Role        `boil:"RoleNameRole" json:"RoleNameRole" toml:"RoleNameRole" yaml:"RoleNameRole"`
	StorekeeperInvoices InvoiceSlice `boil:"StorekeeperInvoices" json:"StorekeeperInvoices" toml:"StorekeeperInvoices" yaml:"StorekeeperInvoices"`
}
...
func (r *userR) GetRoleNameRole() *Role {
	if r == nil {
		return nil
	}
	return r.RoleNameRole
}

func (r *userR) GetStorekeeperInvoices() InvoiceSlice {
	if r == nil {
		return nil
	}
	return r.StorekeeperInvoices
}
...
var (
	userAllColumns            = []string{"id", "name", "email", "password", "role_name", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"name", "email", "password", "role_name"}
...
// RoleNameRole pointed to by the foreign key.
func (o *User) RoleNameRole(mods ...qm.QueryMod) roleQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"name\" = ?", o.RoleName),
	}
...
// StorekeeperInvoices retrieves all the invoice's Invoices with an executor via storekeeper_id column.
func (o *User) StorekeeperInvoices(mods ...qm.QueryMod) invoiceQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"invoices\".\"storekeeper_id\"=?", o.ID),
	)

	return Invoices(queryMods...)
}

// LoadRoleNameRole allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (userL) LoadRoleNameRole(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	if singular {
		...
		args = append(args, object.RoleName)
	...
	if singular {
		...
		object.R.RoleNameRole = foreign
		if foreign.R == nil {
			foreign.R = &roleR{}
		}
		foreign.R.RoleNameUsers = append(foreign.R.RoleNameUsers, object)
		return nil
	}

	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.RoleName == foreign.Name {
				local.R.RoleNameRole = foreign
				if foreign.R == nil {
					foreign.R = &roleR{}
				}
				foreign.R.RoleNameUsers = append(foreign.R.RoleNameUsers, local)
				break
			}
		}
	}
...
// LoadStorekeeperInvoices allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadStorekeeperInvoices(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`invoices`),
		qm.WhereIn(`invoices.storekeeper_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.StorekeeperInvoices = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &invoiceR{}
			}
			foreign.R.Storekeeper = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.StorekeeperID {
				local.R.StorekeeperInvoices = append(local.R.StorekeeperInvoices, foreign)
				if foreign.R == nil {
					foreign.R = &invoiceR{}
				}
				foreign.R.Storekeeper = local
				break
			}
		}
	}
...
// SetRoleNameRole of the user to the related item.
// Sets o.R.RoleNameRole to related.
// Adds o to related.R.RoleNameUsers.
func (o *User) SetRoleNameRole(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Role) error {
	var err error
	if insert {
	...
	updateQuery := fmt.Sprintf(
		...
		strmangle.SetParamNames("\"", "\"", 1, []string{"role_name"}),
	...
	o.RoleName = related.Name
	if o.R == nil {
		o.R = &userR{
			RoleNameRole: related,
		}
	} else {
		o.R.RoleNameRole = related
	}

	if related.R == nil {
		related.R = &roleR{
			RoleNameUsers: UserSlice{o},
		}
	} else {
		related.R.RoleNameUsers = append(related.R.RoleNameUsers, o)
	}

	return nil
}

// AddStorekeeperInvoices adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.StorekeeperInvoices.
// Sets related.R.Storekeeper appropriately.
func (o *User) AddStorekeeperInvoices(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Invoice) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.StorekeeperID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			StorekeeperInvoices: related,
		}
	} else {
		o.R.StorekeeperInvoices = append(o.R.StorekeeperInvoices, related...)
	}
